...
class ExercisesScreen extends StatelessWidget {
            ...
            itemBuilder: (context, index) {
              ...
              return Dismissible(
                ...
                onDismissed: (direction) {
                  exercisesProvider.deleteExercise(index);
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('${exercise.name} supprimé')));
                },
                child: ListTile(
                  title: Text(exercise.name),
                  leading: exercise.isCustomImage
                      ? Image.file(File(exercise.imagePath),
                          width: 50, height: 50)
                      : Image.asset(exercise.imagePath, width: 50, height: 50),
                  trailing: IconButton(
                    icon: const Icon(Icons.delete),
                    onPressed: () => _showDeleteConfirmationDialog(
                        context, exercisesProvider, index, exercise.name),
                  ),
                ),
              );
            },
  ...
  void _showDeleteConfirmationDialog(BuildContext context,
      ExercisesProvider provider, int index, String exerciseName) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Confirmer la suppression'),
          content: Text('Êtes-vous sûr de vouloir supprimer $exerciseName ?'),
          actions: [
            TextButton(
            ...
            TextButton(
              ...
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('$exerciseName supprimé')));
              },
            ),
          ],
        );
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(builder: (context, setState) {
          return AlertDialog(
            title: const Text('Ajouter un exercise'),
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  TextField(
                    decoration:
                        const InputDecoration(labelText: 'Nom de l\'exercise'),
                  ...
                  Slider(
                    value: isTemps
                        ? temps.inSeconds.toDouble()
                        : repetitions.toDouble(),
                    min: 1,
                    max: isTemps ? 300 : 100,
                    divisions: isTemps ? 60 : 99,
                    label: isTemps
                        ? temps.inSeconds.toString()
                  ...
                  const SizedBox(height: 20),
                  Text(
                      'Pause entre séries: ${pauseEntreSeries.inSeconds} secondes'),
                  Slider(
            ...
            actions: [
              ...
              TextButton(
                ...
                onPressed: () {
                  if (name.isNotEmpty &&
                      (selectedImageIndex != null || customImagePath != null)) {
